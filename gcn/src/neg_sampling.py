...
class DynamicSampler(object):
    ...
    def __init__(self):
        ...
        self.K = 3 # number of samples to return
        self.GR = 3 # growth rate
        self.tot_weight = 0.5 # allocated for higher order neighbors
    ...
    def grow(self, i):
        for _ in range(self.GR): # try to add self.GR neighbors
            try:
                ...
                if take != i and take not in self.neighbor_map[i] and take not \
    ...
    def get_higher_order_neighbors(self, i):
        if not self.higher_neigh_ids[i]: # no higher order neighbors
            return [int(len(self.neighbor_map)*random.uniform(0, 1)) \
                    for _ in range(self.K)], [0 for _ in range(self.K)]
        ...
        weights = []
        for idx in idxs:
            weights.append(self.tot_weight*(1/self.higher_neigh_orders[i][idx])/tot_len)
        return ids, weights
